import { handleCreate, handleGetAll, handleGetSingle, handleDelete } from "../helpers/baseHelpers.js";
...
const httpRegisterInstitution = async (req, res) => {
  ...
  handleCreate(Institution, newInstitution, res);
};
...
const httpGetInstitution = async (req, res) => {
  ...
  const institution = await handleGetSingle(Institution, id);
   ...
   (!institution) ? res.status(404).json({ status: "Fail", message: "Institution Not Found!" })
...
const httpDeleteInstitution = async (req, res) => {
  ...
  const institution = await handleGetSingle(Institution, id);
  if (!institution) {
    return res.status(400).json({status: "Fail", message: "Invalid institution id"})
  }
  const isDeleted = await handleDelete(Institution, id)
  if(isDeleted.acknowledged)  res.status(200).json({message: `${institution.name} removed successfully`});
};
